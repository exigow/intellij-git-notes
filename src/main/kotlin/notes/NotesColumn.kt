...
internal class NotesColumn : VcsLogCustomColumn<List<Note>> {
    ...
    override val localizedName = "Notes"
    ...
    private class NotesCellRenderer : ColoredTableCellRenderer(), VcsLogCellRenderer {
        override fun customizeCellRenderer(
            ...
            append(notes.countLabel(), table.applyHighlighters(this, row, column, hasFocus, selected))
        }

        override fun getPreferredWidth() = VcsLogCellRenderer.PreferredWidth.FromData { table, value, _, _ ->
            @Suppress("UNCHECKED_CAST")
            val notes = value as? List<Note>
            if (notes.isNullOrEmpty()) null
            else table.getFontMetrics(table.font).stringWidth(notes.countLabel()) + JBUI.scale(16)
        }
